...
import {danishList, dutchList, portugeseList, russianList, englishList, frenchList, spanishList, italianList} from "./utils.js"
...
async function start() {
   ...
   const lang = await page.evaluate(()=>{
    ...
    if(!lang) {
        console.log("No Language can not be detected")
        process.exit(1)
    }
...
function getFilteredAttribute(links, linkedText ){
    ...
    for(const text of linkedText) {
        const legal = links.filter(t => t.text.toLowerCase().includes(text.toLowerCase())) 
